...
# Run once on the Asus TUF after cloning the repo. Installs only what's
...
try {
    ...
    Write-Fail "Node not installed. Install nvm-windows + 'nvm install lts'"
    exit 1
}
...
if (Test-Path $vsPath) {
    ...
    Write-Warn "VS Build Tools not found. Download: https://visualstudio.microsoft.com/visual-cpp-build-tools/ — install 'Desktop development with C++' workload with Windows SDK."
}
...
# 6. electron-rebuild (native modules rebuilt against Electron's ABI)
